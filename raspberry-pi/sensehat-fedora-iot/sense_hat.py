...
class SenseHat(object):
    ...
    def set_pixel_raw(self, x,y, color, bgcolor=(0,0,0)):
        ''' Color is in the form ( RED, GREEN, BLUE ), where each value x is 0 <= x < 64 '''
        ''' bgcolor 0xc0,0xc0,0xc0 '''
        self.i2cbus.write_byte_data(self.ctrl_address, x*24+y, (color[0]>>2)|bgcolor[0])
        self.i2cbus.write_byte_data(self.ctrl_address, x*24+y+8, (color[1]>>2)|bgcolor[1])
        self.i2cbus.write_byte_data(self.ctrl_address, x*24+y+16, (color[2]>>2))
        #self.i2cbus.write_byte_data(self.ctrl_address, x*24+y, (color[0]>>2)|bgcolor[0])
        #self.i2cbus.write_byte_data(self.ctrl_address, x*24+y+8, (color[1]>>2)|bgcolor[1])
        #if y==7: self.i2cbus.write_byte_data(self.ctrl_address, x*24+y+16, (color[2]>>2))
        #else: self.i2cbus.write_byte_data(self.ctrl_address, x*24+y+16, (color[2]>>2)|bgcolor[2])
        return False
    ...
    def set_pixels(self, pixel_list):
        ...
        for index, pix in enumerate(pixel_list):
            x=index//8
            y=index%8
            index=self._pix_map.get(self._rotation)[x][y]
            x=index//8
            y=index%8
            self.set_pixel_raw(x,y, pix)
    ...
    def get_pixels(self):
        ...
        for x in range(8):
             for y in range(8):
                 pixel_list.append(
                     [self.i2cbus.read_byte_data(self.ctrl_address, x*24+y), self.i2cbus.read_byte_data(self.ctrl_address, x*24+y+8), self.i2cbus.read_byte_data(self.ctrl_address, x*24+y+16)]
                 )
        return pixel_list
    ...
    def get_pixel(self, x, y):
        ...
        return [self.i2cbus.read_byte_data(self.ctrl_address, x*24+y), self.i2cbus.read_byte_data(self.ctrl_address, x*24+y+8), self.i2cbus.read_byte_data(self.ctrl_address, x*24+y+16)]
