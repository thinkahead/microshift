...
stick_file = None #io.open(get_stick(),'rb')
...
def idle_work():
  global last_hf_time, last_lf_time, last_jo_time, lf_enabled, hf_enabled, jo_enabled
  now = time.time()
  ...
  if lf_enabled and (now-last_lf_time > lf_interval):
    ...
    last_lf_time = now
  if jo_enabled and (now-last_jo_time > jo_interval):
    #print("In jo_enabled",now,last_jo_time)
    process_joystick()
    last_jo_time = now
...
def process_joystick():
  ...
  up, down, left, right, push = SH.get_state()
  value=2
  if up: print ("K%s%s"%('U',value))
  if left: print ("K%s%s"%('L',value))
  if down: print ("K%s%s"%('D',value))
  if right: print ("K%s%s"%('R',value))
  if push: print ("K%s%s"%('E',value))
